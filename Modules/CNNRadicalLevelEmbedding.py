...
from Utils.paths import radical_path, radical_eng_path

with open(radical_eng_path, 'r') as f:
...
def char2radical(c):
    if c =='<unk>':
        return ['○']
    # print(f"Running char2radical[{c}]")
    if not contains_eng(c):
        if c in char_info.keys():
            c_info = char_info[c]
            # print('\t'.join([c] + c_info).strip(), file=new_file)
            return list(c_info[0])
    else:
        c_info = searchRadical(c)
        # print(f"c:{c}\nc_info:{c_info}\nlen(c_info):{len(c_info)}")
        # exit(0)
        if len(c_info):
            # return list(c_info)
            return c_info
